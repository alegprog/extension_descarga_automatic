...
function inyectarPanel() {
    ...
    // Cargar Configuración Inicial y crear Device ID si no existe
    chrome.storage.local.get(['apiUrl', 'apiToken', 'deviceId'], function(result) {
        ...
        // Gestionar el Device ID exclusivo de la extensión
        if(result.deviceId) {
    ...
    document.getElementById('btnApiLogin').addEventListener('click', async () => {
        ...
        try {
            const response = await fetch(`${url}/api/login`, {
                ...
                body: JSON.stringify({ 
                    username: user, 
                    password: pass, 
                    deviceId: localDeviceId, // Usamos el ID generado por la extensión
                    licenseKey: license      // Enviamos la licencia (si la escribió)
                })
            });
            ...
            if (data.status) {
                // Guardamos token y reseteamos la vista de licencia por si estaba abierta
                ...
                // El backend pide licencia: Mostramos el campo
            ...
            mostrarEstado("❌ Error de red. ¿El servidor está encendido?");
        } finally {
            btnLogin.disabled = false;
            // Si el cuadro de licencia no está visible, devolvemos el texto original
...
async function procesarDescargas(tipo) {
    ...
    // BLOQUEO ABSOLUTO: Si no hay token de sesión, NADIE puede descargar NADA
    if (!apiToken) {
